...
BEGIN
    ...
    BEGIN
        ...
        SET @SQL = 'SELECT @NullCount_OUT = COUNT(*) FROM staging.MDM_Assets WHERE ' + QUOTENAME(@ColumnName) + ' IS NULL;
                    SELECT @ValueLength_OUT = MAX(LEN(' + QUOTENAME(@ColumnName) + ')) FROM staging.MDM_Assets;
                    SELECT @DistinctCount_OUT = COUNT(DISTINCT ' + QUOTENAME(@ColumnName) + ') FROM staging.MDM_Assets;';
        ...
        PRINT 'Null Count: ' + CAST(@NullCount AS NVARCHAR);
        PRINT 'Max Value Length: ' + CAST(@ValueLength AS NVARCHAR);
        PRINT 'Distinct Value Count: ' + CAST(@DistinctCount AS NVARCHAR);
        PRINT '------------------------';
